...
try:
    ...
    try:
        from data_processing.data_quality import run_all_quality_checks
        QUALITY_CHECKS_AVAILABLE = True
    except ImportError:
...
if __name__ == "__main__":
    ...
    if not hasattr(psutil, 'virtual_memory'):
        logging.warning("psutil module not available or missing virtual_memory function. Install with 'pip install psutil' for memory-efficient processing.")
    
    # Import cleanup functions
    try:
        from src.utils.config import cleanup_all_data_directories, DEFAULT_RETENTION_DAYS, DEFAULT_KEEP_LATEST
    except ImportError:
        # Try importing without the src prefix
        from utils.config import cleanup_all_data_directories, DEFAULT_RETENTION_DAYS, DEFAULT_KEEP_LATEST
    ...
    parser.add_argument('--skip-quality-checks', action='store_true', help='Skip additional data quality checks')
    parser.add_argument('--min-minutes', type=int, default=10, help='Minimum minutes threshold for low-minute players')
    parser.add_argument('--cleanup', action='store_true', help='Clean up old data files after processing')
    parser.add_argument('--retention-days', type=int, default=DEFAULT_RETENTION_DAYS, help='Maximum age in days to keep files when cleaning up')
    parser.add_argument('--keep-latest', type=int, default=DEFAULT_KEEP_LATEST, help='Minimum number of latest files to keep when cleaning up')
    ...
    if args.cleanup and result is not None:
        ...
        if args.dry_run:
            logging.info(f"Dry run: {deleted_count} files would be deleted")
        else:
