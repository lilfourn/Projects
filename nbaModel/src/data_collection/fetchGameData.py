...
# If .env doesn't exist in project root, try the parent directory
...
def fetch_all_player_game_stats(player_data_file, season="2025", num_workers=3, output_dir=None):
    """
    ...
    Args:
        ...
        output_dir (str, optional): Directory to save the game stats CSV file. Defaults to None.
    """
...
def main():
    ...
    project_root = os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))))
    player_info_dir = os.path.join(project_root, "data", "player_info")
    game_stats_dir = os.path.join(project_root, "data", "game_stats")
    
    # Create directories if they don't exist
    os.makedirs(player_info_dir, exist_ok=True)
    os.makedirs(game_stats_dir, exist_ok=True)
    
    save_to_csv(player_data, player_info_dir)
    
    # Fetch game stats for all players
    current_date = datetime.now().strftime("%Y%m%d")
    player_data_file = os.path.join(player_info_dir, f"player_info_{current_date}.csv")
    fetch_all_player_game_stats(player_data_file, season="2025", num_workers=3, output_dir=game_stats_dir)
